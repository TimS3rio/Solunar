package com.timserio.home_domain.use_case

class GetLocationName {
    private val stateToAbbreviationMap = mapOf(
        "Alabama" to "AL",
        "Alaska" to "AK",
        "Alberta" to "AB",
        "American Samoa" to "AS",
        "Arizona" to "AZ",
        "Arkansas" to "AR",
        "Armed Forces (AE)" to "AE",
        "Armed Forces Americas" to "AA",
        "Armed Forces Pacific" to "AP",
        "British Columbia" to "BC",
        "California" to "CA",
        "Colorado" to "CO",
        "Connecticut" to "CT",
        "Delaware" to "DE",
        "District Of Columbia" to "DC",
        "Florida" to "FL",
        "Georgia" to "GA",
        "Guam" to "GU",
        "Hawaii" to "HI",
        "Idaho" to "ID",
        "Illinois" to "IL",
        "Indiana" to "IN",
        "Iowa" to "IA",
        "Kansas" to "KS",
        "Kentucky" to "KY",
        "Louisiana" to "LA",
        "Maine" to "ME",
        "Manitoba" to "MB",
        "Maryland" to "MD",
        "Massachusetts" to "MA",
        "Michigan" to "MI",
        "Minnesota" to "MN",
        "Mississippi" to "MS",
        "Missouri" to "MO",
        "Montana" to "MT",
        "Nebraska" to "NE",
        "Nevada" to "NV",
        "New Brunswick" to "NB",
        "New Hampshire" to "NH",
        "New Jersey" to "NJ",
        "New Mexico" to "NM",
        "New York" to "NY",
        "Newfoundland" to "NF",
        "North Carolina" to "NC",
        "North Dakota" to "ND",
        "Northwest Territories" to "NT",
        "Nova Scotia" to "NS",
        "Nunavut" to "NU",
        "Ohio" to "OH",
        "Oklahoma" to "OK",
        "Ontario" to "ON",
        "Oregon" to "OR",
        "Pennsylvania" to "PA",
        "Prince Edward Island" to "PE",
        "Puerto Rico" to "PR",
        "Quebec" to "PQ",
        "Rhode Island" to "RI",
        "Saskatchewan" to "SK",
        "South Carolina" to "SC",
        "South Dakota" to "SD",
        "Tennessee" to "TN",
        "Texas" to "TX",
        "Utah" to "UT",
        "Vermont" to "VT",
        "Virgin Islands" to "VI",
        "Virginia" to "VA",
        "Washington" to "WA",
        "West Virginia" to "WV",
        "Wisconsin" to "WI",
        "Wyoming" to "WY",
        "Yukon Territory" to "YT"
    )

    operator fun invoke(city: String?, state: String?): String {
        val stateAbbreviation = stateToAbbreviationMap[state] ?: state
        return if (stateAbbreviation.isNullOrBlank()) {
            if (city.isNullOrBlank()) "" else city
        } else {
            if (city.isNullOrBlank()) state ?: "" else "$city, $stateAbbreviation"
        }
    }
}
